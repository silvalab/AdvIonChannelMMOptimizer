...
# ./run N(Number of states) Model_Number(ID of the model of within N to optimize) Times (initial starts) Version
...
#./run 3 3 10 test restart

if [ $# -eq 5 ] 
then
	echo "restart"
	./run solver.txt Sample_Data/protocolsWT.lst Sample_Data/validationWT.lst $1 State$1/State$1parsedDT4CLT4.txt $2 joe-kuo-other-4.5600 $3 $4 State$1/$4/Model$2/iter_state.txt
else
	
	./run solver.txt Sample_Data/protocolsWT.lst Sample_Data/validationWT.lst $1 State$1/State$1parsedDT4CLT4.txt $2 joe-kuo-other-4.5600 $3 $4
fi
